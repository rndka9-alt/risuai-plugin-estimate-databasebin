...
export const STYLES = [
  ...
  '.chr-row { cursor:pointer; }',
  '.chr-row:hover { background:var(--btn); }',
  '.chr-arr { display:inline-block; width:1em; font-size:.8em; }',
  '.sub-indent { padding-left:1.4em; font-size:.85em; }',
  '.sub-info { font-style:italic; }',
  'tr.chr-sub > td { border-bottom-color:transparent; padding-top:2px; padding-bottom:2px; }',
  ...
  '.env-toggle { display:flex; justify-content:center; margin-top:12px; }',
  '.env-btn { background:var(--btn); border:1px solid var(--border2); color:var(--text2); padding:6px 16px; cursor:pointer; font-size:.85em; transition:background .15s; }',
  '.env-btn:first-child { border-radius:6px 0 0 6px; }',
  '.env-btn:last-child { border-radius:0 6px 6px 0; }',
  '.env-btn + .env-btn { border-left:none; }',
  '.env-btn:not(.env-active):hover { background:var(--border2); }',
  '.env-active { background:var(--accent); color:#fff; border-color:var(--accent); }',
